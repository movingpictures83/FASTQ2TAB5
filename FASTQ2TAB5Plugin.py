...
class FASTQ2TAB5Plugin:
  def input(self, inputfile):
       self.parameters = PyIO.readParameters(inputfile)
       self.mate1 = PyPluMA.prefix()+"/"+self.parameters["mate1"]
       self.mate2 = PyPluMA.prefix()+"/"+self.parameters["mate2"]
       self.affix = self.parameters["affix"]
  ...
  def output(self,outputfile):
    ...
    filePathForMate1 = self.mate1
    filePathForMate2 = self.mate2
    filePathForMate1.strip()
    filePathForMate2.strip()

    affixForOutputFile = self.affix 
    ...
    writer = csv.writer(open(outputFile, "w"), delimiter='\t', quoting=csv.QUOTE_NONE, lineterminator="\n")
